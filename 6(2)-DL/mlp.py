...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h1 = X @ W1 + b1  # Linear transformation for the first layer
        z1 = relu(h1)  # Apply ReLU activation

        h2 = z1 @ W2 + b2  # Linear transformation for the second layer
        z2 = relu(h2)  # Apply ReLU activation

        h3 = z2 @ W3 + b3  # Linear transformation for the third layer
        z3 = relu(h3)  # Apply ReLU activation

        h4 = z3 @ W4 + b4  # Linear transformation for the output layer
        y_hat = sigmoid(h4)  # Apply sigmoid activation for binary classification
        y_hat = y_hat.reshape(-1)  # Reshape to match (N,)
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            y_hat, cache = self.forward_propagation(X_train)  # Forward pass
            loss = self.compute_loss(y_hat, y_train, L2_norm)  # Compute loss
            grad = self.back_propagation(cache, X_train, y_train, L2_norm)  # Backpropagation
    ...
    def predict(self, X):
        ...
        predictions = (y_hat >= 0.5).astype(int)  # Binary classification: threshold at 0.5
        ##################
        return predictions
